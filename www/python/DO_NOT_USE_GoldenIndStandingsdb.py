...
def main():
    ...
    connection = pymysql.connect(host='ces.cyjhywszjezw.us-east-1.rds.amazonaws.com',
                             user='cesuser',
    ...
    cur = connection.cursor()
    query = "DELETE FROM `gtresults`"
    cur.execute(query)

    query = "ALTER TABLE `gtresults` AUTO_INCREMENT = 1"
    cur.execute(query)
    ...
    for l in Races.keys():
        ...
        query = "SELECT * FROM `raceresults` WHERE raceid='%s' AND category IN ('PRO MEN','PRO/OPEN WOMEN','MASTER PRO MEN 40+','JUNIOR PRO MEN U21' )" % (str(Races[l]["raceid"]))
        cur.execute(query)
    ...
    for k in Standing.keys():
        ...
        countraces = 8 - int( Category[Standing[k]["category"]]["dropraces"] )

        Standing[k]["total"] = PointsTotal(a,countraces)  
        Results.append([Standing[k]["category"],Standing[k]["ridername"],Standing[k]["riderid"],
    ...
    Results.sort(key=sort_category, reverse=True)

    # Print the results 
    outputfile = open("Output/GT_Results.csv","w")
    ...
    for i in Results:
        ...
        query = "INSERT INTO `gtresults` (`category`,`name`,`riderid`,`total`,`place`,`raceresults`,`r1`,`r2`,`r3`,`r4`,`r5`,`r6`,`r7`,`r8`) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)" 
        #print query
        data = (i[0],i[1],i[2],i[3],Place,raceresults,i[4],i[5],i[6],i[7],i[8],i[9],i[10],i[11])
