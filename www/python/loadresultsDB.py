...
def main():
    ...
    query = "SELECT * FROM category ORDER BY sortorder DESC "
    cesdb_cur.execute(query)
    ...
    query = "SELECT a.name, a.riderid, a.category, b.sortorder, a.ranktotal, a.penalty, a.total, \
                    a.s1, a.s2, a.s3, a.s4, a.s5, a.s6, a.s7, a.s8 \
             FROM raceresults a, categories b \
             WHERE a.category=b.name \
             ORDER BY b.sortorder, a.ranktotal "
    ...
    for row in racedb_cur.fetchall():
        ...
        query = "INSERT INTO raceresults \
                    (raceid,riderid,category,name,place,totaltime,ttotal,stages,penalty,\
                     s1,s2,s3,s4,s5,s6,s7,s8,t1,t2,t3,t4,t5,t6,t7,t8) \
                 VALUES \
