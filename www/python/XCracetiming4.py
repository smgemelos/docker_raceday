...
def loadCategories(cur):
    ...
    for row in cur.fetchall():
        ...
        name = row['name']
        Stages[name] = row['stages']
        Times[name] = row['starttime']
...
def calcStageTime(start,finish):
    ...
    t1 = 0 if (start == "") else start
    t2 = 0 if (finish == "") else finish
...
def validgaptime(time1,time2):
    gaptime = 100 # requires gap time in seconds
...
def main():
    ...
    catStages, catStartTime = loadCategories(racedb_cur)

    laststamp = d.datetime(1970, 1,1).strftime("%Y-%m-%d %H:%M:%S")


    regriders = loadRiders(racedb_cur)


    for rider in regriders:


        riderid = regriders[rider]['riderid']
        sicard_id = regriders[rider]['sicard_id']
        ridername = regriders[rider]['ridername']
        category = regriders[rider]['category']
        plate = regriders[rider]['plate']

        startTime = (catStartTime[category].replace(year=1970, month=01, day=01) - d.datetime(1970, 1,1)).total_seconds()

        zero = 0.0

        query = "INSERT INTO `raceresults` (`plate`,`sicard_id`,`name`,`riderid`,`category`,`ranktotal`,`t1`,`t11`,`t2`,`t22`,`t3`,`t33`,`t4`,`t44`,`t5`,`t55`,`t6`,`t66`,`t7`,`t77`,`t8`,`t88`) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)" 
        data = (plate,sicard_id,ridername,riderid,category,rankstart,startTime,zero,zero,zero,zero,zero,zero,zero,zero,zero,zero,zero,zero,zero,zero,zero)
    ...
    while True:
        ...
        for stamp in stamps:
            ...
            print "Rider: ",ridername,"  SIcardID: ",sicard_id

            if (control_code in Gates) and ( control_mode in cModes ):

                

                catStart = catStartTime[category].replace(year=1970, month=01, day=01) + d.timedelta(minutes=(5))

                if punch_time < catStart:
                    ptime = time.strftime("%H:%M:%S",punch_time.timetuple())
                    ctime = time.strftime("%H:%M:%S",catStartTime[category].timetuple())
                    print "ERROR: Timestamp %s.%s before race category start %s, SIcardID %s" % (ptime,punch_ms,ctime,sicard_id)
                    ERROR = 1
                ...
                if not ERROR: 
                    ...
                    if (count == 0):
                        ...
                        '''
                        If the riderid does not currently exist in the RaceResult db - 
                        but the rider is in the Rider table - add a new row to the RacResults table
                        '''
                        ...
                        if (sicard_id != results["sicard_id"]):
                            ...
                            print "Enter Y to update the record, Enter N to cancel update:"
                            resp = raw_input("?")
                            if (resp == "Y") or (resp == "y"):
                                ERROR = 0
                            else:
            ...
            if not ERROR:
                ...
                if control_code in Gates:
                    ...
                    if ( control_mode in cModes ):
                        ...
                        startTime = (catStartTime[cat].replace(year=1970, month=01, day=01) - d.datetime(1970, 1,1)).total_seconds()

                        Standing['1'] = startTime

                        if (Standing['11'] == 0) and validgaptime(Standing['1'],time2):
                            Standing['11'] = time2
                            Standing['2'] = time2
                        elif (Standing['s1'] != "") and (Standing['22'] == 0.0) and validgaptime(Standing['2'],time2):
                            Standing['22'] = time2
                            Standing['3'] = time2
                        elif (Standing['s2'] != "") and (Standing['33'] == 0.0) and validgaptime(Standing['3'],time2):
                            Standing['33'] = time2
                            Standing['4'] = time2
                        elif (Standing['s3'] != "") and (Standing['44'] == 0.0) and validgaptime(Standing['4'],time2):
                            Standing['44'] = time2
                            Standing['5'] = time2
                        elif (Standing['s4'] != "") and (Standing['55'] == 0.0) and validgaptime(Standing['5'],time2):
                            Standing['55'] = time2
                            Standing['6'] = time2
                        elif (Standing['s5'] != "") and (Standing['66'] == 0.0) and validgaptime(Standing['6'],time2):
                            Standing['66'] = time2
                            Standing['7'] = time2
                        elif (Standing['s6'] != "") and (Standing['77'] == 0.0) and validgaptime(Standing['7'],time2):
                            Standing['77'] = time2
                            Standing['8'] = time2
                        elif (Standing['s7'] != "") and (Standing['88'] == 0.0) and validgaptime(Standing['8'],time2):
                            Standing['88'] = time2
                        ...
                        data = (Standing['total'],Standing['rank'],
                                Standing['s1'],Standing['s2'],Standing['s3'],Standing['s4'],Standing['s5'],Standing['s6'],Standing['s7'],Standing['s8'],
                                Standing['1'],Standing['11'],Standing['2'],Standing['22'],Standing['3'],Standing['33'],Standing['4'],Standing['44'],
                                Standing['5'],Standing['55'],Standing['6'],Standing['66'],Standing['7'],Standing['77'],Standing['8'],Standing['88'],
                                riderid)

                        query = "UPDATE `raceresults` SET `total`='%s',`ranktotal`=%s,`s1`='%s',`s2`='%s',`s3`='%s',`s4`='%s',`s5`='%s',`s6`='%s',`s7`='%s',`s8`='%s',`t1`='%s',`t11`='%s',`t2`='%s',`t22`='%s',`t3`='%s',`t33`='%s',`t4`='%s',`t44`='%s',`t5`='%s',`t55`='%s',`t6`='%s',`t66`='%s',`t7`='%s',`t77`='%s',`t8`='%s',`t88`='%s' WHERE `riderid`='%s' " % data
                       # print query
                        racedb_cur.execute(query)  
