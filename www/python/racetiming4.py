        ...
        for stamp in stamps:
            ...
            if (count == 0):
                ...
                if (count == 0):
                    ...
                    '''
                    If the sicard_id does not currently exist in the RaceResult db - but the rider is in the Rider table - add a new row to the RacResults table
                    '''
                    zero = 0.0

                    query = "INSERT INTO `raceresults` (`plate`,`sicard_id`,`name`,`riderid`,`raceid`,`category`,`ranktotal`,`t1`,`t11`,`t2`,`t22`,`t3`,`t33`,`t4`,`t44`,`t5`,`t55`,`t6`,`t66`,`t7`,`t77`,`t8`,`t88`,`dnf`,`dq`) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)" 
                    data = (plate,sicard_id,ridername,riderid,raceid,category,rankstart,zero,zero,zero,zero,zero,zero,zero,zero,zero,zero,zero,zero,zero,zero,zero,zero,'N','N')
                ...
                if not ERROR:
                    ...
                    if control_code in Gates:
                        ...
                        if ( control_mode in cModes ):
                            ...
                            query = "UPDATE `raceresults` SET `total`=%s,`ranktotal`=%s,`s1`=%s,`s2`=%s,`s3`=%s,`s4`=%s,`s5`=%s,`s6`=%s,`s7`=%s,`s8`=%s,`t1`=%s,`t11`=%s,`t2`=%s,`t22`=%s,`t3`=%s,`t33`=%s,`t4`=%s,`t44`=%s,`t5`=%s,`t55`=%s,`t6`=%s,`t66`=%s,`t7`=%s,`t77`=%s,`t8`=%s,`t88`=%s WHERE `sicard_id`=%s "
